...
//     const destination = role === 'student' ? '/home-student' : '/teacher-home';
    
//     // Execute login
//     await loginWithEmail(email, password, navigate, destination);
//     setLoading(false);
//   };
...
//               type="submit"
//               disabled={loading}
//               className={`w-full bg-white text-black py-5 rounded-3xl font-black text-2xl border-[4px] border-black shadow-[8px_8px_0px_0px_#3b82f6] hover:bg-[#3b82f6] hover:text-white hover:border-white hover:shadow-[8px_8px_0px_0px_#fff] transition-all transform hover:-translate-y-1 active:scale-95 uppercase italic ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
//             >
...
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { loginWithEmail } from '../utils/auth/authActions.js';  // make sure this calls handleSuccessfulLogin
...
const Login = () => {
  ...
  const [password, setPassword] = useState('');
  const [loading, setLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState(''); // optional: show nice errors
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // loginWithEmail should now call handleSuccessfulLogin internally
      await loginWithEmail(email, password, navigate);
      // No need to pass destination anymore — handled inside authActions
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#121212] flex flex-col items-center justify-center font-mono p-4 relative overflow-hidden select-none text-white">
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        <div className="bg-black border-[6px] border-white rounded-[40px] p-8 shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] relative overflow-hidden">
          ...
          {errorMsg && (
            <div className="bg-red-900/50 border border-red-500 text-red-200 px-4 py-3 rounded-xl mb-6 text-center font-bold">
              {errorMsg}
            </div>
          )}
        ...
        {/* Bottom Link – consider linking to signup if you implement it */}
        <p className="text-center mt-10 font-black text-xs uppercase tracking-widest text-gray-500">
          Not in the database? <span className="text-white underline cursor-pointer hover:text-[#facc15]">Contact Admin</span>
        </p>
      </div>
